...
class VyewAPI {
	...
	*/
	...
	function VyewAPI($apikey, $secret, $apiurl="https://vyew.com/api/api.php"){
		$this->apikey = $apikey;
		$this->secret = $secret;
		$this->apiurl = $apiurl;
		if($this->debuglog){
    ...
    {
    	$args=array( 'email'=>$email, 'pass'=>$pass, 'fname'=>$fname, 'lname'=>$lname );
...
/** func: getusers
     ...
     *                      - *has_conf_call* - User can display the conference call number in theri meetings
     *
     * Returns:
     *  {array} - With these items:
     *      - code {number} - status or error code. (1=OK, <1=Error)
